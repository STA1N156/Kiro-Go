...
func TestClaudeToKiroInjectsThinkingPrompt(t *testing.T) {
	...
	// 系统指令（含 thinking_mode）现在通过 history 头部 priming 注入，不再污染 CurrentMessage.Content
	combined := collectPayloadSystemText(payload)
	if !strings.Contains(combined, "<thinking_mode>enabled</thinking_mode>") {
		t.Fatalf("missing thinking_mode tag in payload system priming: %q", combined)
	}
	if !strings.Contains(combined, "<max_thinking_length>5000</max_thinking_length>") {
		t.Fatalf("budget not in payload system priming: %q", combined)
	}
}
...
func TestResolveAndConvertEndToEnd(t *testing.T) {
	...
	payload := ClaudeToKiro(req, prompt)
	priming := collectPayloadSystemText(payload)
	...
	for _, must := range []string{
		...
		if !strings.Contains(priming, must) {
			t.Fatalf("missing fragment %q in payload system priming:\n%s", must, priming)
		}
	}
	if !strings.Contains(current, "证明哥德巴赫猜想") {
		t.Fatalf("user content missing in current message: %q", current)
	}
}

// collectPayloadSystemText 提取 priming history 中的系统指令文本（user role 部分），
// 用于断言 thinking_mode 等系统级 prompt 是否被正确注入。
func collectPayloadSystemText(payload *KiroPayload) string {
	var b strings.Builder
	for _, h := range payload.ConversationState.History {
		if h.UserInputMessage != nil {
			b.WriteString(h.UserInputMessage.Content)
			b.WriteString("\n")
		}
	}
	return b.String()
...
func TestOpenAIConversationIDRandomForSyntheticAnchor(t *testing.T) {
	...
	payloadA := OpenAIToKiro(req, "")
	payloadB := OpenAIToKiro(req, "")
