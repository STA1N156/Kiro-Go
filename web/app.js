...
(() => {
  ...
  // API Keys management (multi-key support)
  async function loadApiKeys() {
    ...
    list.innerHTML = d.keys.map(k => {
      const date = new Date(k.createdAt * 1000).toLocaleDateString();
      const safeName = (k.name || '').replace(/"/g, '&quot;');
      return `<div class="apikey-row" data-id="${k.id}">
        <span class="apikey-name">${escapeHtml(k.name || '')}</span>
        <code class="apikey-preview">${escapeHtml(k.keyPreview || '')}</code>
        <span class="apikey-date">${date}</span>
        <label class="switch apikey-toggle" data-id="${k.id}" data-name="${safeName}">
          <input type="checkbox" ${k.enabled ? 'checked' : ''} />
          <span class="slider"></span>
        </label>
        <button class="btn btn-danger btn-sm apikey-delete" data-id="${k.id}" aria-label="delete">
          <i class="fa-solid fa-trash" aria-hidden="true"></i>
        </button>
      </div>`;
    }).join('');
    list.querySelectorAll('.apikey-toggle input').forEach(input => {
      input.addEventListener('change', e => {
        const wrap = e.target.closest('.apikey-toggle');
        toggleApiKey(wrap.dataset.id, e.target.checked, wrap.dataset.name);
      });
    });
    list.querySelectorAll('.apikey-delete').forEach(btn => {
  ...
  async function createApiKey() {
    ...
    try {
      const res = await api('/apikeys', { method: 'POST', body: JSON.stringify({ name }) });
      const d = await res.json();
      if (d.key) {
        nameInput.value = '';
        try { await copyText(d.key); } catch (e) { /* ignore */ }
        await showKeyCreatedDialog(d.key);
        loadApiKeys();
      } else {
        toast(d.error || t('common.saveFailed'), 'error');
      }
    } catch (e) {
      toast((e && e.message) || t('common.saveFailed'), 'error');
    }
  }
  async function toggleApiKey(id, enabled, name) {
    try {
      await api('/apikeys/' + encodeURIComponent(id), { method: 'PUT', body: JSON.stringify({ name, enabled }) });
    } catch (e) {
      toast((e && e.message) || t('common.saveFailed'), 'error');
    }
  }
  async function deleteApiKey(id) {
    const ok = await confirmAction(t('settings.confirmDeleteKey'), {
      title: t('settings.apiSettings'),
      confirmText: t('common.delete') || t('common.confirm'),
      variant: 'danger'
    });
    if (!ok) return;
    try {
  ...
  async function toggleRequireApiKey() {
    const requireApiKey = $('requireApiKey').checked;
    try {
      await api('/settings', { method: 'POST', body: JSON.stringify({ requireApiKey }) });
    } catch (e) {
      toast((e && e.message) || t('common.saveFailed'), 'error');
    }
  }
  async function showKeyCreatedDialog(key) {
    await confirmAction(key, {
      title: t('settings.keyCreated'),
      confirmText: t('common.copied') || t('common.confirm'),
      cancelText: t('common.close') || t('common.cancel')
    });
  }
  function escapeHtml(s) {
  ...
  async function loadLogs(page) {
    ...
    try {
      const res = await api('/logs?limit=' + logsPageSize + '&page=' + logsCurrentPage);
      const d = await res.json();
      const tbody = $('logsBody');
      const empty = $('logsEmpty');
      const pagination = $('logsPagination');
      if (!tbody || !empty || !pagination) return;
      if (d.retentionDays) {
        const sel = $('logRetentionSelect');
        if (sel) sel.value = String(d.retentionDays);
      }
      logsTotalPages = d.totalPages || 1;
      ...
      tbody.innerHTML = d.logs.map(log => {
        ...
        const timeStr = time.toLocaleTimeString([], { hour12: false }) + ' ' + time.toLocaleDateString([], { month: '2-digit', day: '2-digit' });
        const statusBadge = log.success
          ? '<span class="badge badge-success">' + t('logs.success') + '</span>'
          : '<span class="badge badge-error" title="' + escapeHtml(log.error || '') + '">' + t('logs.failed') + '</span>';
        const tokens = log.success ? (formatNum(log.inputTokens || 0) + '/' + formatNum(log.outputTokens || 0)) : '-';
        const duration = log.duration > 0
          ? (log.duration >= 1000 ? (log.duration / 1000).toFixed(1) + 's' : log.duration + 'ms')
          : '-';
        const account = log.account ? maskEmail(log.account) : '-';
        const keyName = log.keyName || '-';
        const apiType = (log.apiType || '-') + (log.stream ? ' stream' : '');
        return '<tr>' +
          '<td class="logs-cell-time">' + escapeHtml(timeStr) + '</td>' +
          '<td class="logs-cell-model">' + escapeHtml(log.model || '-') + '</td>' +
          '<td>' + escapeHtml(account) + '</td>' +
          '<td class="logs-cell-key">' + escapeHtml(keyName) + '</td>' +
          '<td><span class="badge badge-info">' + escapeHtml(apiType) + '</span></td>' +
  ...
  function renderLogsPagination() {
    ...
    if (!container) return;
    if (logsTotalPages <= 1) { container.innerHTML = ''; return; }
    let html = '';
    html += '<button class="btn btn-outline btn-xs" data-page="' + (logsCurrentPage - 1) + '" ' +
      (logsCurrentPage <= 1 ? 'disabled' : '') + '>' + t('logs.prev') + '</button>';
    let start = Math.max(1, logsCurrentPage - 2);
    let end = Math.min(logsTotalPages, start + 4);
    start = Math.max(1, end - 4);
    for (let i = start; i <= end; i++) {
      const cls = i === logsCurrentPage ? 'btn btn-primary btn-xs' : 'btn btn-outline btn-xs';
      html += '<button class="' + cls + '" data-page="' + i + '">' + i + '</button>';
    }
    html += '<button class="btn btn-outline btn-xs" data-page="' + (logsCurrentPage + 1) + '" ' +
      (logsCurrentPage >= logsTotalPages ? 'disabled' : '') + '>' + t('logs.next') + '</button>';
    html += '<span class="logs-page-info">' + t('logs.pageInfo').replace('{0}', logsCurrentPage).replace('{1}', logsTotalPages) + '</span>';
    container.innerHTML = html;
    container.querySelectorAll('button[data-page]').forEach(btn => {
      btn.addEventListener('click', () => {
        const p = parseInt(btn.dataset.page, 10);
        if (!isNaN(p) && p >= 1 && p <= logsTotalPages) loadLogs(p);
      });
    });
  }
  async function clearLogs() {
    const ok = await confirmAction(t('logs.confirmClear'), {
      title: t('logs.title'),
    ...
    try {
      ...
      loadLogs(1);
      toast(t('logs.cleared') || t('detail.saved'), 'success');
    } catch (e) {
      toast((e && e.message) || t('common.failed'), 'error');
    }
  }
  async function updateLogRetention() {
    ...
    try {
      await api('/logs/retention', { method: 'POST', body: JSON.stringify({ days }) });
  ...
  function bindSettingsEvents() {
    ...
    $('newKeyName').addEventListener('keypress', e => { if (e.key === 'Enter') createApiKey(); });
    $('requireApiKey').addEventListener('change', toggleRequireApiKey);
    ...
    if (refreshLogsBtn) refreshLogsBtn.addEventListener('click', () => loadLogs(logsCurrentPage));
    const clearLogsBtn = $('clearLogsBtn');
    if (clearLogsBtn) clearLogsBtn.addEventListener('click', clearLogs);
    const logRetention = $('logRetentionSelect');
    if (logRetention) logRetention.addEventListener('change', updateLogRetention);
  }
